...
SDS:动态字符串:预分配冗余空间减少内存的频繁分配.字符串长度小于1M时,扩容是加倍现有空间,大于1M时,扩容时只会扩容1M的空间,字符串的最大长度为512M
...
操作: zadd key score value、zrange key 0 -1、zrevrange key 0 -1、zscore key value、zrank key value、

​		 zrangebyscore key score1 score2、zrangebyscore key score1 socre2 withscores、zrem key value
